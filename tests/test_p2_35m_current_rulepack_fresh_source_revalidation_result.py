...
def test_p2_35m_is_current_rulepack_revalidation_without_combined_independent_holdout():
    c=_y(CURRENT); v=c["current_rulepack_validation"]; assert c["current_evidence_id"]=="p2-35m-current-rulepack-fresh-source-revalidation-current-detection-evidence"
    assert v["current_revalidation_id"]=="p2-35m-current-rulepack-fresh-source-revalidation"; assert v["fresh_attack_revalidation_after_current_rule_change"]=="COMPLETED"; assert v["fresh_benign_revalidation_after_current_rule_change"]=="COMPLETED"; assert v["fresh_current_rulepack_performance_available"] is True
    m=c["post_remediation_revalidations"][-1]; assert m["revalidation_id"]=="p2-35m-current-rulepack-fresh-source-revalidation"; assert m["fixture_count"]==10 and m["hits"]==6 and m["expected_technique_matches"]==1; assert m["benign_parsed_events"]==425974 and m["benign_flagged_events"]==4; assert m["independent_source_family_holdout"] is False
