...
DETECTOR_COMMIT = "456b2a8238837026c8249ae1020b16249715bc04"
EXPECTED_RULE_TREE = "b42725734f70cfc87ccd0122e6ab90da2169d7599b2abaaf22c27050efa0c4d2"
...
def verify_inputs(control: Path, detector: Path, source: Path) -> tuple[dict, dict]:
    ...
    if sha256(manifest) != str(binding["source_manifest_sha256"]):
                ...
                json.dumps(
                    {
                        ...
                        "label": "ignore",
                        "expected_techniques": [],
                        "notes": "P2-11D external calibration; no independent event-level ground truth.",
                    },
                    ensure_ascii=False,
                )
                + "\n"
...
def main() -> int:
    ...
    if freeze_data["rules_tree_sha256"] != EXPECTED_RULE_TREE:
        raise RuntimeError(
            f"runtime rules hash mismatch: {freeze_data['rules_tree_sha256']}"
        )
    ...
    for scenario_id in EXPECTED_ORDER:
        ...
        manifest_data = {
            ...
            "provenance": {
                ...
                "notes": "Same 12 scenarios already observed in P2-11B; this run is external_calibration after tuning.",
            },
            "files": files,
            "scenarios": [
                {
    ...
    for item in prepared:
        ...
        peak_memory_max = max(
            peak_memory_max,
            float(result["performance"]["peak_memory_mb"]),
        )
        outcomes.append(
            {
    ...
    aggregate = {
        "schema": "breachscope.p2_11d_external_calibration_result.v1",
        "evaluation_class": "external_calibration",
