...
DETECTOR_COMMIT = "e876ce32c6b5c9846ca553b03a291548a7a7aa93"
EXPECTED_RULE_TREE = "1626aca8b7b9a8e2a7e1f42360c65b504827ff43e52159eb3716613ebb540a50"
EXPECTED_RULE_COUNT = 64
EXPECTED_PREVIOUS_FINDINGS = 89
EXPECTED_PREVIOUS_FLAGGED_EVENTS = 85
                ...
                json.dumps(
                    {
                        ...
                        "label": "ignore",
                        "expected_techniques": [],
                        "notes": "P2-11G external calibration; no independent event-level ground truth.",
                    },
                    ensure_ascii=False,
                )
                + "\n"
...
def main() -> int:
    ...
    run(
        sys.executable, str(evaluator), "freeze",
        "--repo", str(detector),
        "--rules-dir", str(detector / "rules"),
        "--out", str(freeze), cwd=detector,
    )
    ...
    # P2-11G changes ATT&CK annotation/evaluation semantics only. The YAML rule
    # tree is intentionally unchanged from P2-11F and the focused test proves
    # R-ENC still emits exactly one finding for the same predicate.
    ...
    for scenario_id in EXPECTED_ORDER:
        ...
        run(
            sys.executable, str(evaluator), "index",
            "--manifest", str(manifest_path),
            "--corpus-root", str(scenario_dir),
            "--out", str(index_path), cwd=detector,
        )
    ...
    for item in prepared:
        ...
        run(
            ...
            "--freeze", str(freeze),
            "--rules-dir", str(detector / "rules"),
            "--out", str(result_path), cwd=detector,
        )
    ...
    if totals["findings"] != EXPECTED_PREVIOUS_FINDINGS:
        raise RuntimeError(
            f"mapping-only change altered findings: expected {EXPECTED_PREVIOUS_FINDINGS}, got {totals['findings']}"
        )
    if totals["flagged_events"] != EXPECTED_PREVIOUS_FLAGGED_EVENTS:
        raise RuntimeError(
            "mapping-only change altered flagged events: "
            f"expected {EXPECTED_PREVIOUS_FLAGGED_EVENTS}, got {totals['flagged_events']}"
        )

    hits = sum(row["status"] == "hit" for row in outcomes)
    misses = len(outcomes) - hits
    aggregate = {
        "schema": "breachscope.p2_11g_external_calibration_result.v1",
        "evaluation_class": "external_calibration",
        "change_class": "technique_mapping_and_evaluator_semantics_only",
        "predicate_change": False,
        "rules_tree_changed_from_p2_11f": False,
        ...
        "previous_p2_11f_findings": EXPECTED_PREVIOUS_FINDINGS,
        "previous_p2_11f_flagged_events": EXPECTED_PREVIOUS_FLAGGED_EVENTS,
        ...
        "claim_boundary": {
            ...
            "note": "Same public Atomic-EVTX scenarios were already observed in P2-11B and used for calibration; P2-11G changes ATT&CK mapping/evaluator semantics, not detection predicates.",
        },
    }
