...
SCHEMA = "breachscope.p2_35c_socbed_timeline_one_pass.v1"
...
RUN_ATTACK_RE = re.compile(r"\bevent=run_attack\b")
ATTACK_RE = re.compile(r"\battack=([A-Za-z0-9_.-]+)\b")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run preregistered P2-35C SOCBED timeline alignment measurement."
    )
...
def main() -> int:
    ...
    if sys.version_info[:2] != (3, 11):
        raise SystemExit("P2-35C requires Python 3.11")
    if sys.platform != "win32":
        raise SystemExit("P2-35C requires Windows")
