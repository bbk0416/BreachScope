...
ANALYSIS_ID = "p2-35l-current-rulepack-fresh-exact-bytes-v1"
SCHEMA = "breachscope.p2_35l_current_rulepack_fresh_exact_bytes.v1"
...
def global_lock_path() -> Path:
    return (
        ...
        / ".breachscope"
        / "canonical_locks"
        / "P2_35L_CURRENT_RULEPACK_FRESH_EXACT_BYTES.lock"
    )
...
def verify_attack_sources(
    attack_dir: Path,
    contract: dict[str, Any],
) -> list[dict[str, Any]]:
    expected_rows = contract["attack_revalidation"]["datasets"]
    expected = {row["filename"] for row in expected_rows}
...
def run(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        payload["claim_boundary"] = {
            ...
            "same_attack_source_family_as_historical_revalidation": True,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Run P2-35L fresh exact-byte current-rulepack "
