...
def scan_one(args: tuple[str, str]) -> dict[str, object]:
    root_s, path_s = args
    root = Path(root_s)
    path = Path(path_s)
    count = 0
...
def main() -> int:
    if len(sys.argv) != 4:
        raise SystemExit("usage: p2_13b_bind_win11_benign_v2.py ARCHIVE EXTRACTED_ROOT OUTPUT_JSON")
    ...
    with ProcessPoolExecutor(max_workers=workers) as pool:
        rows = list(pool.map(scan_one, [(str(root), str(p)) for p in evtx_files], chunksize=1))
    rows.sort(key=lambda x: str(x["path"]))

    total_records = sum(int(x["records"]) for x in rows)
    parse_errors = sum(int(x["parse_errors"]) for x in rows)
    sysmon_records = sum(int(x["records"]) for x in rows if bool(x["sysmon_by_path"]))
    ...
    print(f"P2_13B_PARALLEL_WORKERS={workers}")
