...
EXPECTED_ARCHIVE_SHA256 = "377f8cba5db95a453a3ee8bd19f493efafc23724541482a4da99da28ee4665f9"
EXPECTED_ARCHIVE_BYTES = 43033041
EXPECTED_EVENTS = 587286
EXPECTED_RULES = 66
EXPECTED_MEMBER = "apt29_evals_day2_manual_2020-05-02035409.json"
...
def main() -> int:
    ...
    assert binding["evaluation_class"] == "confirmatory_same_campaign_holdout_pre_scoring"
    assert binding["source_binding"]["archive_sha256"] == EXPECTED_ARCHIVE_SHA256
    assert binding["actual_archive_parse"]["total_events"] == EXPECTED_EVENTS
    assert binding["actual_archive_parse"]["parse_errors"] == 0
    assert binding["emulation_plan_binding"]["unique_technique_count"] == 41
    assert binding["protocol_state"]["archive_bytes_bound_before_detection"] is True
    assert binding["protocol_state"]["day2_labels_bound_before_detection"] is True
    assert binding["protocol_state"]["detector_executed_on_day2"] is False
    assert binding["protocol_state"]["tuning_after_day1_before_day2_forbidden"] is True
    ...
    source_labels = {
        str(x).upper() for x in binding["emulation_plan_binding"]["technique_ids"]
    }
    ...
    result = {
        "schema": "breachscope.p2_12g_confirmatory_holdout_result.v1",
        "evaluation_class": "confirmatory_same_campaign_holdout",
        "selection_id": "p2-12e-otrf-apt29-day2-confirmatory-v1",
        "binding_id": "p2-12f-otrf-apt29-day2-v1",
        ...
        "claim_boundary": {
            "fresh_external_corpus": False,
            "independent_external_source_from_day1": False,
            "same_campaign_as_day1": True,
            "different_reserved_day_split": True,
            ...
            "note": "Day 2 is a separately reserved same-campaign confirmatory holdout. Exact legacy ATT&CK-ID overlap is a corpus-level source-label overlap measure, not event-level recall or a production detection rate. No post-result legacy-to-modern ATT&CK remapping is applied.",
        },
    }
