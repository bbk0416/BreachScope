...
SCHEMA = "breachscope.p2_21_independent_one_pass_measurement.v1"
...
def preflight(args: argparse.Namespace) -> int:
    ...
    expected = binding["runner"]["sha256"]
    if payload["runner_sha256"] != expected:
...
def run(args: argparse.Namespace) -> int:
    ...
    if actual_runner != binding["runner"]["sha256"]:
    ...
    for source in binding["datasets"]:
        ...
        try:
            ...
            row["source_verification"] = {
                ...
                "sha256": sha256(path),
                "size_match": actual_size == source["size_bytes"],
                "git_blob_match": actual_blob == source["git_blob_sha1"],
            }
            if not row["source_verification"]["size_match"] or not row["source_verification"]["git_blob_match"]:
            ...
            events, parse_errors = load_evtx(path, parser, Event)
            row.update(
                stage="parsed",
                events=len(events),
            ...
            row.update(
                ...
                scenario_techniques=sorted(
                    {t for s in scenarios for t in (getattr(s, "mitre_techniques", None) or [])}
                ),
    ...
    payload["summary"] = {
        "dataset_count": total,
        "hits": hits,
        "misses_or_errors": total - hits,
        "path_label_dataset_hit_rate": hits / total if total else 0.0,
