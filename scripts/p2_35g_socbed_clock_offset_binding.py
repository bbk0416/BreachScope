...
SCHEMA = "breachscope.p2_35g_socbed_clock_offset_binding.v1"
ANALYSIS_ID = "p2-35g-socbed-clock-offset-binding-v1"
...
def oracle_match(action: str, event: Any) -> bool:
    ...
    if action == "misc_execute_malware":
        exact_command = command_line.strip().strip('"').strip()
        return (
            event_id == "1"
            and exact_command == r"c:\windows\meterpreter_bind_tcp.exe"
        )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run preregistered P2-35G posthoc clock-offset binding audit."
    )
...
def main() -> int:
    ...
    if sys.version_info[:2] != (3, 11):
        raise SystemExit("P2-35G requires Python 3.11")
    if sys.platform != "win32":
        raise SystemExit("P2-35G requires Windows")
    ...
    try:
        ...
        for action, cfg in contract["oracle_actions"].items():
            ...
            pre_roll_seconds = int(contract["timeline"]["pre_roll_seconds"])
            corrected_start = window["start"] - timedelta(seconds=pre_roll_seconds)
            oracle_events = []
            for event in events:
                ts = predecessor._parse_timestamp(event.timestamp)
                if not (corrected_start <= ts < window["end"]):
                    continue
                if oracle_match(action, event):
                    oracle_events.append(event)

            expected_known = int(cfg["known_posthoc_predicate_match_count"])
            if len(oracle_events) != expected_known:
                raise RuntimeError(
                    f"posthoc-known oracle count mismatch for {action}: "
                    f"{len(oracle_events)} != {expected_known}"
                )
            ...
            oracle_results[action] = {
                ...
                "original_window_start": window["start_timestamp"],
                "corrected_window_start": corrected_start.isoformat(),
                "window_end": window["end_timestamp"],
                "pre_roll_seconds": pre_roll_seconds,
                "known_posthoc_predicate_match_count": expected_known,
        ...
        result.update(
            {
                ...
                "claim_boundary": {
                    **dict(contract["claim_boundary"]),
                    "source_derived_oracle_event_binding": (
