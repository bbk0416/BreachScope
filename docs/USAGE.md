...
**선택 패키지**:
- `reportlab>=4` - 한글 PDF 리포트 생성
- `weasyprint>=62` - HTML→PDF 폴백 렌더링
...
- `--pdf`: 고객 제출용 한글 PDF 리포트 생성 (ReportLab 권장, WeasyPrint 폴백)
...
- `--pdf` 옵션 사용 시
- `out/report.pdf` 생성
- Risk Score, 경영진 요약, 우선 조치 권고, Top Findings, 호스트 위험도, 타임라인, ATT&CK 커버리지, IOC 요약 포함
- 한글 폰트는 자동 탐지하며 `BS_PDF_FONT_REGULAR`, `BS_PDF_FONT_BOLD`로 직접 지정 가능
- WeasyPrint 패키지 필요
