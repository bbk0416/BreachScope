...
v8부터 웹 콘솔 좌측에 최근 분석 이력이 표시됩니다. 분석이 끝나면 `case_id`가 자동 발급되고, 기본적으로 `~/.breachscope/cases` 아래에 산출물이 보존됩니다.

- **열기**: 이전 케이스의 Risk/타임라인/ATT&CK/호스트 위험도 미리보기를 다시 표시
- **다운로드**: `/api/cases/{case_id}/report` 경로로 HTML/JSON/CSV/IOC/룰/Manifest/ZIP 산출물 다운로드
- **삭제**: 케이스 인덱스에서 제거하고 안전한 작업 디렉토리만 파일 삭제
