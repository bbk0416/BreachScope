...
from .reporting import build_summary, render_html, maybe_render_pdf, export_json as export_json_func, export_csv as export_csv_func, export_iocs_csv, export_manifest, export_case_package
from .rulepack import summarize_rules, export_rule_catalog_csv
...
class Pipeline:
    ...
    def build_report(self) -> Report:
        ...
        rule_pack_summary = summarize_rules(self.rules or [])
        summary["rules"] = {"count": len(self.rules) if self.rules else 0}
        summary["rule_pack"] = rule_pack_summary
        summary["filters"] = {
    ...
    def export_report(
        ...
        out_iocs = out_prefix.with_suffix(".iocs.csv")
        out_rule_catalog = out_prefix.with_suffix(".rules.csv")
        out_pdf = out_prefix.with_suffix(".pdf")
        out_manifest = out_prefix.with_suffix(".manifest.json")
        ...
        if render_pdf:
            ...
            if maybe_render_pdf(out_html, out_pdf, self.report):
                generated_artifacts.append(out_pdf)
        ...
        if export_csv:
            export_csv_func(self.report, out_csv, redact=redact)
            generated_artifacts.append(out_csv)

        # 대응자/관제자가 바로 복사해 쓸 수 있게 IOC 후보는 별도 산출물로 항상 생성합니다.
        export_iocs_csv(self.report, out_iocs)
        generated_artifacts.append(out_iocs)

        # 룰팩 커버리지 검토와 외부 리뷰를 위해 적용 규칙 카탈로그도 항상 생성합니다.
        export_rule_catalog_csv(self.rules or [], out_rule_catalog)
        generated_artifacts.append(out_rule_catalog)

        export_manifest(self.report, out_manifest, artifact_paths=generated_artifacts)
        generated_artifacts.append(out_manifest)
        export_case_package(out_package, generated_artifacts)
