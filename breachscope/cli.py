...
def main():
    ...
    if args.validate_rules:
        ...
        for r in rules:
            mt = f" ({r.mitre_technique})" if r.mitre_technique else ""
            print(f"- {r.id}: {r.name}{mt}")
        return
    ...
    if args.validate_input:
        if args.demo:
            ...
            if not args.input:
                ap.error("--input 또는 --demo 중 하나를 지정하세요.")
    ...
    if args.demo:
        ...
        if not args.input and not args.collect_evtx:
            ap.error("--input, --demo, 또는 --collect-evtx 중 하나를 지정하세요.")
        if args.input:
            input_dir = Path(args.input)
        elif not args.collect_evtx:
            ap.error("--input 또는 --demo를 지정하세요.")
